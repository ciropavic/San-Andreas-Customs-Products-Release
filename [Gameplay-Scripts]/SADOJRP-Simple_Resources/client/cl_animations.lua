...
if enableDrunk then
	RegisterCommand("drunk", function(source, args, rawCommand)
		if not isDrunk then
			...
			DisplayNotification('~rYou are now drunk!')
			isDrunk = true
		else
...
if enableUnlock then
	...
	RegisterNetEvent("SARP:adminunlock")
  AddEventHandler("SARP:adminunlock", function()
    ...
    exports['mythic_notify']:DoHudText('success', 'Successfully Admin Unlocked')
  end)
end
