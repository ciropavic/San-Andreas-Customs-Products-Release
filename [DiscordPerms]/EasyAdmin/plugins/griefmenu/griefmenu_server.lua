...
RegisterServerEvent("EasyAdmin:griefmenu:fakecall")
AddEventHandler("EasyAdmin:griefmenu:fakecall", function(playerId, type)
	if DoesPlayerHavePermission(source,"griefing.fakenotif") then
		TriggerClientEvent("EasyAdmin:showNotification", source, getName(playerId).." will hear the sound now.")
		SendWebhookMessage(moderationNotification, getName(source).." triggered **Fake "..fakeNotifications[type].name.."** on "..getName(playerId).. "", "troll")
		TriggerClientEvent("EasyAdmin:griefmenu:fakecall", playerId, type)
	end
end)

RegisterServerEvent("EasyAdmin:griefmenu:BlowTyres")
AddEventHandler("EasyAdmin:griefmenu:BlowTyres", function(playerId, type)
	if DoesPlayerHavePermission(source,"griefing.veh") then
		SendWebhookMessage(moderationNotification, getName(source).." triggered **Blow Tyres** on "..getName(playerId).. "", "troll")
		TriggerClientEvent("EasyAdmin:griefmenu:BlowTyres", playerId, type)
	end
end)

RegisterServerEvent("EasyAdmin:griefmenu:Eject")
AddEventHandler("EasyAdmin:griefmenu:Eject", function(playerId, type)
	if DoesPlayerHavePermission(source,"griefing.veh") then
		SendWebhookMessage(moderationNotification, getName(source).." triggered **Eject** on "..getName(playerId).. "", "troll")
		TriggerClientEvent("EasyAdmin:griefmenu:Eject", playerId, type)
	end
end)
