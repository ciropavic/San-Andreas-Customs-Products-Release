...
AddEventHandler("EasyAdmin:BuildPlayerOptions", function(playerId) -- BuildPlayerOptions is triggered after building options like kick, ban.. Passes a Player ServerId
	...
	if not permissions["griefing.menu"] then return end
	...
	local effectsMenu = _menuPool:AddSubMenu(thisGriefMenu,"Effects","",true) -- Submenus work, too!
	effectsMenu:SetMenuWidthOffset(menuWidth)

	if permissions["griefing.truck"] then
		local thisItem = NativeUI.CreateItem('"The Truck Punchline"',"")
		effectsMenu:AddItem(thisItem) -- Items dont require a trigger.
	...
	if permissions["griefing.fakenotif"] then
		local sl = {}
		for i, effect in pairs(fakeNotifications) do
			table.insert(sl, effect.name)
		end

		local thisItem = NativeUI.CreateListItem("Fake Sound", sl, 1, "plays fake sound effect for 10 seconds")
		effectsMenu:AddItem(thisItem)
		thisItem.OnListSelected = function(sender, item, index)
			if item == thisItem then
				i = item:IndexToItem(index)
				TriggerServerEvent("EasyAdmin:griefmenu:fakecall", playerId, index)
			end
		end
	end

	local vehicleOptions = _menuPool:AddSubMenu(thisGriefMenu,"Vehicle Options","",true) 
	vehicleOptions:SetMenuWidthOffset(menuWidth)

	if permissions["griefing.veh"] then
		local thisItem = NativeUI.CreateItem("Blow Tyres","")
		vehicleOptions:AddItem(thisItem) -- Items dont require a trigger.
		thisItem.Activated = function(ParentMenu,SelectedItem)
			TriggerServerEvent("EasyAdmin:griefmenu:BlowTyres", playerId)
		end


		local thisItem = NativeUI.CreateItem("Eject from Car","")
		vehicleOptions:AddItem(thisItem) -- Items dont require a trigger.
		thisItem.Activated = function(ParentMenu,SelectedItem)
			TriggerServerEvent("EasyAdmin:griefmenu:Eject", playerId)
		end
	...
	local dangerousMenu = _menuPool:AddSubMenu(thisGriefMenu,"~r~Dangerous ~s~Options","",true) 
	dangerousMenu:SetMenuWidthOffset(menuWidth)


	if permissions["griefing.lag"] then
		local thisItem = NativeUI.CreateItem("~y~Lag Game","")
		dangerousMenu:AddItem(thisItem) -- Items dont require a trigger.
		thisItem.Activated = function(ParentMenu,SelectedItem)
...
RegisterNetEvent("EasyAdmin:griefmenu:fakecall")
AddEventHandler("EasyAdmin:griefmenu:fakecall", function(type)
	local notif = fakeNotifications[type]
...
RegisterNetEvent("EasyAdmin:griefmenu:BlowTyres")
AddEventHandler("EasyAdmin:griefmenu:BlowTyres", function()
...
RegisterNetEvent("EasyAdmin:griefmenu:Eject")
AddEventHandler("EasyAdmin:griefmenu:Eject", function()
